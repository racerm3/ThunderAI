...
`js/mzta-spamreport.js` (`taSpamReport` class) wraps `taStorage` for spam report operations: load/save/remove reports, track in-flight analysis state via `browser.storage.session`, enforce a 100-entry limit, and store error states. To keep the spam log populated even when a message is deleted mid-analysis (race condition with Thunderbird custom filters), `_generateSpamReportForMessage()` in `mzta-background.js` captures a metadata snapshot (subject/from/to/message_date) via `_buildReportMetadata()` — preferring full MIME headers but falling back to MessageHeader fields, which survive deletion — and `saveError()` accepts this snapshot as an optional third argument so error entries are not blank. When a message cannot be found at all before analysis (e.g. deleted by the user or a custom filter), the spam filter now cleans up silently (`removeReportData`) and returns without writing a "Message not found" error entry to the spam log.
...
**Auto-block on receive:** when the spam filter runs on an incoming email and `autoMove` is set, the sender's domain is automatically added to the block list and the message permanently deleted when `spamValue` is at/above the threshold. The effective threshold is `Math.max(SpamThreshold, 1)` so a `spamValue` of `0` (AI verdict: not spam) is never auto-blocked even if the user configures a threshold of `0`. `SpamThreshold` is stored in each report (freshly read `prefs.spamfilter_threshold`, preserving a legitimate `0` via a `typeof === 'number'` check; falling back to `prefs_init.spamfilter_threshold`, then default 70).
