...
async function loadSpamReport(){
 ...
 function populateTable(data, sortKey = currentSortState.key, sortDirection = currentSortState.direction) {
  ...
  reportRows.forEach(({ email, report }) => {
      ...
      const spamThreshold = (typeof report.SpamThreshold === 'number') ? report.SpamThreshold : 70;
      tdMoved.textContent = (reportIsSpam(report) ? browser.i18n.getMessage("yes_string") : browser.i18n.getMessage("no_string")) + ` (${spamThreshold})`;
    ...
    document.querySelectorAll(".option-input").forEach(element => {
      ...
      switch (element.type) {
        ...
        case 'number':
          ...
          if(element.id == 'spamfilter_timeout_sec') default_number_value = prefs_default.spamfilter_timeout_sec;
          if(element.id == 'spamfilter_max_retries') default_number_value = prefs_default.spamfilter_max_retries;
